...
/** __Linear__ Monte Carlo solution */
int femSolveMC(void)
{
	int   rv = AF_OK;
  FILE *fw;
  ...
  if ((rv = fem_read_rand_data(femMCfile)) != AF_OK) { goto memFree; }
 	...
 	if ((rv = fem_rand_backup_orig_data()) != AF_OK) { goto memFree; }
 	if ((rv = fem_rand_alloc_res()) != AF_OK) { goto memFree; }


  for (i=0; i<femMCSims; i++)
  {
 	  if ((rv = fem_rand_create_random_data()) != AF_OK) { goto memFree; }

    if (i > 0) /* probably not needed, but.. */
    {
      femMatSetZeroBig(&K);
      femVecSetZeroBig(&F);

      for (j=0; j<resRLen; j++) { resRval[j] = 0.0 ; }
      for (j=0; j<resELen; j++) { resEVal[j] = 0.0 ; }
    ...
    else
    {
 	  ...
 	  if ((rv = fem_rand_add_res()) != AF_OK) { goto memFree; }


    /* some assessment code should be here - saving only for failed data*/
    if (fem_rand_test_fail() == AF_YES)
    {
      fail_num++ ;

      if (femMCSaveMode == 2)
      {
	      if ((rv = femWriteRes(femMCGenFname(fem_output_file(), (i+1), 0))) != AF_OK) { goto memFree; }
      }
    }
  ...
  fem_rand_swap_res1();
	if ((rv = femWriteRes(femMCGenFname(fem_output_file(), 0, 1))) != AF_OK) { goto memFree; }

  fem_rand_swap_res2();
	if ((rv = femWriteRes(femMCGenFname(fem_output_file(), 0, 2))) != AF_OK) { goto memFree; }

  fem_rand_swap_resmax();
	if ((rv = femWriteRes(femMCGenFname(fem_output_file(), 0, 3))) != AF_OK) { goto memFree; }

  fem_rand_swap_resmin();
	if ((rv = femWriteRes(femMCGenFname(fem_output_file(), 0, 4))) != AF_OK) { goto memFree; }

  /* summary file: */
  fw = fopen(fem_output_file(),"w");
  fprintf(fw,"%li %li\n", femMCSims, fail_num);
  fprintf(fw,"%li\n", femMCSaveMode);

  fprintf(fw,"%s\n",femMCGenFname(fem_output_file(), 0, 1));
  fprintf(fw,"%s\n",femMCGenFname(fem_output_file(), 0, 2));

  fprintf(fw,"%s\n",femMCGenFname(fem_output_file(), 0, 3));
  fprintf(fw,"%s\n",femMCGenFname(fem_output_file(), 0, 4));
